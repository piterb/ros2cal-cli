...
def prepare_image(path: str, out_path: str) -> str:
    img = Image.open(path)
    # if width < 1500px, enlarge (numbers will be more readable)
    scale = 2
    if img.width < 1500:
...
def ocr_roster_image(path: str) -> str:
    processed_image_path = prepare_image(path, path + "_processed.png")
    base64_image = encode_image(processed_image_path)
  #print(base64_image[:100] + "...")  # print first 100 characters of the Base64 string
        ...
        input=[
            ...
            },
            {
                ...
                "content": [
                    {
                        "type": "input_text",
                        "text": "Transcribe the roster in this image exactly as text.",
                    },
                    {
                        "type": "input_image",
    ...
    # OCR output is plain text
    ocr_text = resp.output_text
    return ocr_text


def parse_roster_text_to_json(roster_text: str) -> dict:
    resp = client.responses.create(
        model="gpt-5.1",
        temperature=0,
        top_p=1,
...
def parse_roster_image_to_json(path: str) -> dict:
    ocr_text = ocr_roster_image(path)
    # print("--- OCR TEXT ---")
    # print(ocr_text)
    data = parse_roster_text_to_json(ocr_text)
    return data


if __name__ == "__main__":
    image_path = Path(sys.argv[1])
    roster = parse_roster_image_to_json(str(image_path))
    #print(json.dumps(roster, indent=2, ensure_ascii=False))
    out_path = image_path.with_suffix(image_path.suffix + "_out.json")
